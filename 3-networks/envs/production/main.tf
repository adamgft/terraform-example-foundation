...
locals {
   ...
   * Base network ranges
   */
  base_subnet_aggregates    = ["10.0.0.0/19", "10.0.32.0/19", "100.64.0.0/16", "100.65.0.0/16"]
  base_hub_subnet_ranges    = ["10.0.0.0/24", "10.0.32.0/24"]
  base_private_service_cidr = "10.0.88.0/21"
  base_subnet_primary_ranges = {
    (var.default_region1) = "10.0.24.0/21"
    (var.default_region2) = "10.0.56.0/21"
  }
   ...
   */
  restricted_subnet_aggregates    = ["10.0.128.0/19", "10.0.160.0/19", "100.72.0.0/16", "100.73.0.0/16"]
  restricted_hub_subnet_ranges    = ["10.0.128.0/24", "10.0.160.0/24"]
  restricted_private_service_cidr = "10.0.216.0/21"
  restricted_subnet_primary_ranges = {
    (var.default_region1) = "10.0.152.0/21"
    (var.default_region2) = "10.0.184.0/21"
  }
  restricted_subnet_secondary_ranges = {
    (var.default_region1) = [
      {
        range_name    = "rn-${local.environment_code}-shared-restricted-${var.default_region1}-gke-pod"
        ip_cidr_range = "100.72.128.0/19"
      },
      {
        range_name    = "rn-${local.environment_code}-shared-restricted-${var.default_region1}-gke-svc"
        ip_cidr_range = "100.72.160.0/19"
      }
    ]
  }
}
